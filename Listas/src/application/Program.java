...
Fonte:OO e List em Java - Aulão #003
...
//Tipo list=interface.Não é classe.
...
//			Para não consumir a quebra de linha da entrada do id.
			sc.nextLine();
...
//Outra maneira de encontar a posição do id. Ligado a Função Integer position
		// Integer pos = position(list, idsalary);
		
//		Verificar se existe o id através do stream.Aceita funções de alta ordem.
//Função filter,filtra a lista para  somente os elementos solicitados.Funcionários
//com o idSalary e pega o primeiro findFirst.Se não existir retorna nulo.
		Employee emp = list.stream().filter(x -> x.getId() == idsalary).findFirst().orElse(null);
		
//Outra maneira de encontar a posição do id
		// if (pos == null) {
		if (emp == null) {
...
//Para saltar uma linha ao rodar
	...
	// Função auxiliar para procurar elemento na lista.Retorna a posição
	// do elemento na lista.Static,pois o mais é static
...
//Se retornar verdadeiro,encontrou funcionário.
		return emp != null;
	}
}